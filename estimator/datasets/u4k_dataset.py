...
from external.zoedepth.models.base_models.midas import Resize
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            if self.consistency is True:
                ...
                for h_start in self.h_start_list:
                    for w_start in self.w_start_list:
                        ...
                        if self.pre_norm_bbox:
                            bbox = torch.tensor([
                                bbox[0] / self.transform_cfg.image_raw_shape[1] * self.transform_cfg.network_process_size[1], 
                                bbox[1] / self.transform_cfg.image_raw_shape[0] * self.transform_cfg.network_process_size[0], 
                                bbox[2] / self.transform_cfg.image_raw_shape[1] * self.transform_cfg.network_process_size[1], 
                                bbox[3] / self.transform_cfg.image_raw_shape[0] * self.transform_cfg.network_process_size[0]])
                ...
                image_tensor, gt_info, crop_info = random_crop(image_tensor, [depth_gt_tensor, disp_gt_tensor], self.patch_raw_shape)
